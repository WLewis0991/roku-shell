...
from roku_shell.config import IP_FILE
...
def main(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        ...
        if IP_FILE.exists():
            console.print(
                f"[dim]Using Roku IP: {IP_FILE.read_text(encoding='utf-8').strip()}[/dim]"
            )
        else:
            console.print(
                "[dim]No Roku IP configured yet. Run any command to set it up.[/dim]"
            )
