...
def get_or_prompt_ip() -> str:
    ...
    console.print(
        "[bold yellow]No Roku IP address found. Initial setup required.[/bold yellow]"
    )
    ...
    if roku_ip:
        ...
        console.print(
            f"[bold green]✅ IP saved successfully to {IP_FILE}[/bold green]\n"
        )
        return roku_ip

    console.print(
        "[bold red]❌ Error: You must provide an IP address to use Roku Shell.[/bold red]"
    )
